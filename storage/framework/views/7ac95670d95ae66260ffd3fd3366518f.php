<?php $__env->startSection('title', 'تفاصيل المعاملة المالية - ' . config('app.name')); ?>
<?php $__env->startSection('page_title', 'تفاصيل المعاملة المالية'); ?>
...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
            <p class="text-xs font-medium text-muted">المحاسبة · المعاملات · #<?php echo e($transaction->transaction_number ?? $transaction->id); ?></p>
            <h2 class="mt-1 text-2xl font-semibold tracking-tight text-ink md:text-[28px]">معاملة #<?php echo e($transaction->transaction_number ?? $transaction->id); ?></h2>
            <p class="mt-1 text-sm text-muted">
                <i class="fas fa-calendar-alt text-xs"></i>
                أُنشئت في <?php echo e($transaction->created_at->format('Y-m-d H:i')); ?>

            </p>
        </div>
        <div class="admin-hero-actions flex flex-wrap gap-2">
            <a href="<?php echo e(route('admin.transactions.edit', $transaction)); ?>" class="btn-press inline-flex h-9 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-medium text-white">
    ...
    <section class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <article class="rounded-2xl border border-line bg-surface p-4 shadow-soft">
            <div class="inline-flex size-9 items-center justify-center rounded-xl <?php echo e($amountTone === 'text-accent' ? 'bg-accent-soft text-accent' : ($amountTone === 'text-metal' ? 'bg-metal/15 text-metal' : 'bg-canvas-muted text-muted')); ?>">
                <i class="fas fa-coins text-sm"></i>
            </div>
            <p class="mt-3 text-xs text-muted">المبلغ</p>
            <p class="mt-1 text-xl font-semibold tabular-nums <?php echo e($amountTone); ?>">
        ...
        <article class="rounded-2xl border border-line bg-surface p-4 shadow-soft">
            <div class="inline-flex size-9 items-center justify-center rounded-xl <?php echo e($typeMeta['classes'] ?? 'bg-canvas-muted text-muted'); ?>">
                <i class="<?php echo e($typeMeta['icon'] ?? 'fas fa-exchange-alt'); ?> text-sm"></i>
            </div>
            <p class="mt-3 text-xs text-muted">النوع</p>
            <p class="mt-1">
                <?php if($typeMeta): ?>
                    <span class="inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-semibold <?php echo e($typeMeta['classes']); ?>"><?php echo e($typeMeta['label']); ?></span>
                <?php else: ?>
                    <span class="text-sm font-semibold text-ink"><?php echo e($transaction->type); ?></span>
                <?php endif; ?>
            </p>
        </article>
        <article class="rounded-2xl border border-line bg-surface p-4 shadow-soft">
            <div class="inline-flex size-9 items-center justify-center rounded-xl <?php echo e($statusMeta['classes'] ?? 'bg-canvas-muted text-muted'); ?>">
                <i class="fas fa-toggle-on text-sm"></i>
            </div>
            <p class="mt-3 text-xs text-muted">الحالة</p>
            <p class="mt-1">
                <?php if($statusMeta): ?>
                    <span class="inline-flex rounded-lg px-2.5 py-1 text-xs font-semibold <?php echo e($statusMeta['classes']); ?>"><?php echo e($statusMeta['label']); ?></span>
                <?php endif; ?>
            </p>
        </article>
        <article class="rounded-2xl border border-line bg-surface p-4 shadow-soft">
            ...
            <p class="mt-1 text-sm font-semibold text-ink"><?php echo e($transaction->currency ?? 'EGP'); ?></p>
        </article>
    </section>
        ...
        <div class="space-y-5 xl:col-span-2">
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <dl class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
                    <div>
                        ...
                        <dd class="mt-1 text-lg font-semibold tabular-nums <?php echo e($amountTone); ?>">
                            <?php echo e($isDebit ? '-' : ($isCredit ? '+' : '')); ?><?php echo e(number_format($transaction->amount, 2)); ?> ج.م
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-muted">الفئة</dt>
                        <dd class="mt-1 text-sm font-semibold text-ink"><?php echo e($transaction->category ?? 'غير محدد'); ?></dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-muted">تاريخ الإنشاء</dt>
                        <dd class="mt-1 text-sm font-medium tabular-nums text-ink"><?php echo e($transaction->created_at->format('Y-m-d H:i')); ?></dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-muted">آخر تحديث</dt>
                        <dd class="mt-1 text-sm font-medium tabular-nums text-ink"><?php echo e($transaction->updated_at->format('Y-m-d H:i')); ?></dd>
                    </div>
                    <?php if($transaction->createdBy): ?>
                    <div>
                        <dt class="text-xs font-medium text-muted">أنشأها</dt>
                        <dd class="mt-1 text-sm font-semibold text-ink"><?php echo e($transaction->createdBy->name); ?></dd>
                    </div>
                    <?php endif; ?>
                </dl>
            </article>
            ...
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <dl class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
                    <div>
                        ...
                        <dd class="mt-1 text-sm font-semibold text-ink"><?php echo e($transaction->user->name ?? 'غير معروف'); ?></dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-muted">رقم الهاتف</dt>
                        <dd class="mt-1 text-sm font-semibold text-ink"><?php echo e($transaction->user->phone ?? '—'); ?></dd>
                    </div>
                    <?php if($transaction->user->email): ?>
                    <div>
                        <dt class="text-xs font-medium text-muted">البريد الإلكتروني</dt>
                        <dd class="mt-1 break-all text-sm font-medium text-ink"><?php echo e($transaction->user->email); ?></dd>
                    </div>
                    <?php endif; ?>
                    <div>
            ...
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <div class="p-4 sm:p-5">
                    <p class="whitespace-pre-line text-sm leading-relaxed text-ink"><?php echo e($transaction->description); ?></p>
                </div>
            </article>
            <?php endif; ?>
            ...
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <dl class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
                    ...
                    <div>
                        <dt class="text-xs font-medium text-muted"><?php echo e($key); ?></dt>
                        <dd class="mt-1 text-sm font-semibold text-ink"><?php echo e(is_array($value) ? json_encode($value, JSON_UNESCAPED_UNICODE) : $value); ?></dd>
                    </div>
            ...
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <dl class="space-y-4 p-4 sm:p-5">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <dd>
                            <a href="<?php echo e(route('admin.payments.show', $transaction->payment)); ?>" class="text-sm font-semibold text-accent transition hover:text-accent/80">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-xs font-medium text-muted">المبلغ</dt>
                        <dd class="text-sm font-semibold tabular-nums text-ink"><?php echo e(number_format($transaction->payment->amount, 2)); ?> ج.م</dd>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-xs font-medium text-muted">طريقة الدفع</dt>
                        <dd class="text-sm font-medium text-ink"><?php echo e($transaction->payment->payment_method ?? '—'); ?></dd>
                    </div>
                </dl>
            </article>
            <?php endif; ?>
            ...
            <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
                ...
                <dl class="space-y-4 p-4 sm:p-5">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <dd>
                            <a href="<?php echo e(route('admin.invoices.show', $transaction->invoice)); ?>" class="text-sm font-semibold text-accent transition hover:text-accent/80">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <dd class="text-sm font-semibold tabular-nums text-ink"><?php echo e(number_format($transaction->invoice->total_amount, 2)); ?> ج.م</dd>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-xs font-medium text-muted">الحالة</dt>
                        <dd>
                            <span class="inline-flex rounded-lg px-2.5 py-1 text-xs font-semibold <?php echo e($invoiceStatus['classes']); ?>"><?php echo e($invoiceStatus['label']); ?></span>
                        </dd>
                    </div>
                </dl>
            </article>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views/admin/transactions/show.blade.php ENDPATH**/ ?>