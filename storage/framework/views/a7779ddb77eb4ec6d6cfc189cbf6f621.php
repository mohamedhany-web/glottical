            ...
            <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <select name="delivery_type" id="delivery_type" class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-colors">
                        ...
                        <option value="group" <?php echo e(request('delivery_type') == 'group' ? 'selected' : ''); ?>>جماعي</option>
                        <option value="one_to_one" <?php echo e(request('delivery_type') == 'one_to_one' ? 'selected' : ''); ?>>فردي 1:1</option>
                    </select>
                </div>
                <div>
                ...
                <div class="section-card flex flex-col overflow-hidden">
                    <div class="section-card-header flex items-start justify-between gap-3">
                        ...
                        <div class="flex flex-col items-end gap-1 flex-shrink-0">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold <?php echo e($course->is_active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-slate-100 text-slate-700 dark:bg-slate-600 dark:text-slate-300'); ?>">
                                <?php echo e($course->is_active ? 'نشط' : 'معطل'); ?>

                            </span>
                            <?php $isSolo = ($course->delivery_type ?? 'group') === 'one_to_one'; ?>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold <?php echo e($isSolo ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200' : 'bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-200'); ?>">
                                <?php echo e($isSolo ? 'فردي 1:1' : 'جماعي'); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views\admin\advanced-courses\index.blade.php ENDPATH**/ ?>