...
<head>
    ...
    <?php if(is_string($r2PublicBase) && $r2PublicBase !== ''): ?>
        ...
        <?php if($r2Host): ?>
            <link rel="dns-prefetch" href="https://<?php echo e($r2Host); ?>">
            <link rel="preconnect" href="https://<?php echo e($r2Host); ?>" crossorigin>
        <?php endif; ?>
    <?php endif; ?>

    <link rel="preconnect" href="https://images.unsplash.com" crossorigin>
    <?php echo $__env->make('partials.landing.head', ['landingCss' => ['theme']], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <style>
...
<?php echo $__env->make('partials.landing.navbar', ['navActive' => 'home', 'navHero' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

<?php echo $__env->make('partials.welcome-landing-main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <div class="ft-dialog">
        <div class="ft-head">
            <div class="ft-head-copy">
                <p class="ft-kicker">
                    ...
                    Glottical · 30 <?php echo e($isRtl ? 'دقيقة' : 'min'); ?>

                </p>
                <h3 id="ft-title" class="ft-title"><?php echo e(__($a.'.free_trial_modal_title')); ?></h3>
                <p class="ft-sub"><?php echo e(__($a.'.free_trial_modal_sub')); ?></p>
            </div>
            <button type="button" class="ft-close" data-close-free-trial aria-label="<?php echo e(__($a.'.free_trial_close')); ?>">
        ...
        <div class="ft-body">
            <div id="ft-loading" class="ft-loading">
                ...
                <?php echo e(__($a.'.free_trial_loading')); ?>
            ...
            <div id="ft-calendar" class="ft-calendar hidden">
                <div>
                    <div class="ft-row">
                        <p class="ft-label" style="margin:0"><?php echo e(__($a.'.free_trial_pick_date')); ?></p>
                        <p class="ft-muted"><?php echo e($isRtl ? 'خلال أسبوعين' : 'Next 2 weeks'); ?></p>
                    </div>
                    <div class="ft-scroll-wrap" data-ft-scroll-wrap>
                        <button type="button" class="ft-scroll-btn is-prev" data-ft-scroll="-1" aria-label="<?php echo e($isRtl ? 'الأيام السابقة' : 'Previous days'); ?>" disabled>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="<?php echo e($isRtl ? 'm9 18 6-6-6-6' : 'm15 18-6-6 6-6'); ?>"/></svg>
                        </button>
                        <div id="ft-dates" class="ft-hscroll" role="listbox" aria-label="<?php echo e(__($a.'.free_trial_pick_date')); ?>"></div>
                        <button type="button" class="ft-scroll-btn is-next" data-ft-scroll="1" aria-label="<?php echo e($isRtl ? 'الأيام التالية' : 'Next days'); ?>" disabled>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="<?php echo e($isRtl ? 'm15 18-6-6 6-6' : 'm9 18 6-6-6-6'); ?>"/></svg>
                        </button>
                    </div>
                </div>

                <div>
                    <p class="ft-label"><?php echo e(__($a.'.free_trial_pick_time')); ?></p>
                    <div id="ft-times" class="ft-times"></div>
                    <p id="ft-no-times" class="ft-sub hidden"><?php echo e(__($a.'.free_trial_no_slots')); ?></p>
                </div>

                <form id="ft-form" class="ft-form">
                    <input type="hidden" name="starts_at" id="ft-starts-at" required>
                    <div class="ft-field-wrap">
                        <label for="ft-name" class="ft-label"><?php echo e(__($a.'.free_trial_name')); ?></label>
                        <input type="text" name="name" id="ft-name" required autocomplete="name" class="ft-field" value="<?php echo e(auth()->user()->name ?? ''); ?>">
                    </div>
                    <div class="ft-form-grid">
                        <div class="ft-field-wrap">
                            <label for="ft-email" class="ft-label"><?php echo e(__($a.'.free_trial_email')); ?></label>
                            <input type="email" name="email" id="ft-email" autocomplete="email" class="ft-field" value="<?php echo e(auth()->user()->email ?? ''); ?>">
                        </div>
                        <div class="ft-field-wrap">
                            <label for="ft-phone" class="ft-label"><?php echo e(__($a.'.free_trial_phone')); ?></label>
                            <input type="tel" name="phone" id="ft-phone" autocomplete="tel" class="ft-field" value="<?php echo e(auth()->user()->phone ?? ''); ?>">
                        </div>
                    </div>
                    <div class="ft-field-wrap">
                        <label for="ft-goal" class="ft-label"><?php echo e(__($a.'.free_trial_goal')); ?></label>
                        <input type="text" name="goal" id="ft-goal" class="ft-field" placeholder="<?php echo e($isRtl ? 'سفر، عمل، دراسة…' : 'Travel, work, studyâ€¦'); ?>">
                    </div>
                    <button type="submit" id="ft-submit" disabled class="ft-submit">
                        <?php echo e(__($a.'.free_trial_submit')); ?>

                    </button>
                    <p class="ft-muted" style="text-align:center;line-height:1.6"><?php echo e($isRtl ? 'بدون التزام · سنؤكد الموعد برسالة قصيرة' : 'No commitment Â· Weâ€™ll confirm by a short message'); ?></p>
                </form>
            </div>
            ...
            <div id="ft-success" class="ft-success hidden">
                ...
                <h4 class="ft-title"><?php echo e(__($a.'.free_trial_success')); ?></h4>
                <p id="ft-success-msg" class="ft-sub"></p>
                <button type="button" data-close-free-trial class="ft-close" aria-label="<?php echo e(__($a.'.free_trial_close')); ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg>
                </button>
            </div>
        </div>
...
(function(){
    ...
    function freeTrialBooking(){
        ...
        function loadSlots(){
            ...
            fetch(<?php echo json_encode(route('public.free-trial.slots'), 15, 512) ?> + '?days=14', {
                ...
                .then(function(data){
                    ...
                    if (!dates.length && errorEl) {
                        errorEl.textContent = <?php echo json_encode(__($a.'.free_trial_no_slots'), 15, 512) ?>;
                        errorEl.classList.remove('hidden');
                    }
                })
                .catch(function(){
                    if (errorEl) {
                        errorEl.textContent = <?php echo json_encode($isRtl ? 'تعذّر تحميل المواعيد. حاول مرة أخرى.' : 'Could not load slots. Please try again.', 15, 512) ?>;
                        errorEl.classList.remove('hidden');
                    }
                })
                .finally(function(){
        ...
        function formatDateChip(isoDate){
            try {
                ...
                var day = dt.toLocaleDateString(<?php echo json_encode($locale, 15, 512) ?>, { weekday: 'short' });
                var date = dt.toLocaleDateString(<?php echo json_encode($locale, 15, 512) ?>, { day: 'numeric', month: 'short' });
        ...
        function setupDateScroller(scroller){
            ...
            if (prev) prev.addEventListener('click', function(){ scroller.scrollBy({ left: (<?php echo json_encode($isRtl, 15, 512) ?> ? 1 : -1) * Math.max(180, scroller.clientWidth * 0.7), behavior: 'smooth' }); });
            if (next) next.addEventListener('click', function(){ scroller.scrollBy({ left: (<?php echo json_encode($isRtl, 15, 512) ?> ? -1 : 1) * Math.max(180, scroller.clientWidth * 0.7), behavior: 'smooth' }); });
        ...
        function updateScrollEdges(wrap, scroller){
            ...
            if (first && last && canScroll) {
                ...
                var rtl = <?php echo json_encode($isRtl, 15, 512) ?>;
                atStart = rtl
        ...
        form.addEventListener('submit', function(e){
            ...
            document.getElementById('ft-submit').disabled = true;
            fetch(<?php echo json_encode(route('public.free-trial.book'), 15, 512) ?>, {
                method: 'POST',
                headers: {
