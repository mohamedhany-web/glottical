...
unset($__errorArgs, $__bag); ?>
                                ...
                                <div class="space-y-2 md:col-span-2">
                                    ...
                                    <select name="delivery_type" class="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-3 text-slate-800 dark:text-slate-200">
                                        <option value="group" <?php if(old('delivery_type', $advancedCourse->delivery_type ?? 'group') === 'group'): echo 'selected'; endif; ?>>جماعي</option>
                                        <option value="one_to_one" <?php if(old('delivery_type', $advancedCourse->delivery_type ?? 'group') === 'one_to_one'): echo 'selected'; endif; ?>>فردي 1:1 مع معلم</option>
                                    </select>
                                    <p class="text-xs text-slate-500">الكورس الفردي يتطلب اختيار المدرب أدناه.</p>
                                </div>
                                <div class="space-y-2 md:col-span-2">
                                    ...
                                    <select name="billing_mode" class="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-3 text-slate-800 dark:text-slate-200">
                                        <option value="one_time" <?php if(old('billing_mode', $advancedCourse->billing_mode ?? 'one_time') === 'one_time'): echo 'selected'; endif; ?>>دفعة واحدة (وصول دائم)</option>
                                        <option value="monthly" <?php if(old('billing_mode', $advancedCourse->billing_mode ?? 'one_time') === 'monthly'): echo 'selected'; endif; ?>>اشتراك شهري متجدد</option>
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">سعر الاشتراك الشهري (جنيه)</label>
                                    <input type="number" name="monthly_price" value="<?php echo e(old('monthly_price', $advancedCourse->monthly_price)); ?>" min="0" step="0.01"
                                ...
                                <div class="space-y-2">
                                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">سعر شهري بعد الخصم (اختياري)</label>
                                    <input type="number" name="monthly_price_after_discount" value="<?php echo e(old('monthly_price_after_discount', $advancedCourse->monthly_price_after_discount)); ?>" min="0" step="0.01"
                                           class="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-3 text-slate-800 dark:text-slate-200">
                                </div>
                            </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="p-6 sm:p-8 space-y-4 text-sm text-slate-700 dark:text-slate-300">
                            ...
                            <div class="space-y-2">
                                ...
                                <?php if($advancedCourse->thumbnail_url): ?>
                                    <div class="mb-3 space-y-2">
                                        <img src="<?php echo e($advancedCourse->thumbnail_url); ?>" alt="صورة الكورس الحالية"
                                             class="w-full h-32 object-cover rounded-xl border border-slate-200 dark:border-slate-600">
                                        <p class="text-xs text-slate-500 dark:text-slate-400">رابط العرض على الموقع (يُولَّد تلقائياً):</p>
                                        <input type="text" readonly value="<?php echo e($advancedCourse->thumbnail_url); ?>" dir="ltr"
                                               class="w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3 py-2 text-xs text-slate-700 dark:text-slate-300">
                                        <?php if(!\App\Services\CourseThumbnailStorage::isExternalUrl($advancedCourse->thumbnail)): ?>
                                            <p class="text-xs text-slate-400">المسار المحفوظ: <?php echo e($advancedCourse->thumbnail); ?></p>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400">ارفع ملفاً جديداً لاستبدال الصورة الحالية.</p>
                                <div>
                                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">أو رابط صورة خارجي</label>
                                    <input type="url" name="thumbnail_link" value="<?php echo e(old('thumbnail_link', \App\Services\CourseThumbnailStorage::isExternalUrl($advancedCourse->thumbnail) ? $advancedCourse->thumbnail : '')); ?>" dir="ltr" placeholder="https://..."
                                    ...
                                    <?php $__errorArgs = ['thumbnail_link'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="text-xs text-rose-500 mt-1"><?php echo e($message); ?></p> <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                                ...
                                <?php if($advancedCourse->thumbnail): ?>
                                    <label class="inline-flex items-center gap-2 text-sm text-rose-700 cursor-pointer">
                                        ...
                                        <input type="checkbox" name="remove_thumbnail" value="1" class="rounded border-slate-300 text-rose-600">
                                        حذف الصورة الحالية
                                    </label>
                                <?php endif; ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views\admin\advanced-courses\edit.blade.php ENDPATH**/ ?>