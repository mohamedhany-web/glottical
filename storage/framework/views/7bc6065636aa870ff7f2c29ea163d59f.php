...
<head>
    ...
    <meta name="keywords"    content="كورسات ألماني, كورسات إنجليزي, كول سنتر, سوق العمل, ألمانيا, <?php echo e(config('app.name', 'Glottical')); ?>">
    <meta name="author"      content="<?php echo e(config('app.name', 'Glottical')); ?>">
    ...
    <meta property="og:title"            content="<?php echo e(__('public.courses_page_title')); ?> - <?php echo e(config('app.name', 'Glottical')); ?>">
    ...
    <meta property="og:image:alt"        content="كورسات <?php echo e(config('app.name', 'Glottical')); ?>">
    ...
    <meta name="twitter:title"       content="<?php echo e(__('public.courses_page_title')); ?> - <?php echo e(config('app.name', 'Glottical')); ?>">
    ...
    <meta name="twitter:image:alt"   content="كورسات <?php echo e(config('app.name', 'Glottical')); ?>">
...
<body class="font-sans text-white antialiased font-display bg-acad-navy"
      x-data="{
        searchQuery: '',
        selectedCategoryId: <?php echo \Illuminate\Support\Js::from($initialCategoryId)->toHtml() ?>,
    ...
    <div class="container-acad relative z-10">
      <div class="max-w-4xl mx-auto text-center reveal">
        <span class="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs sm:text-sm font-extrabold mb-6 glass-panel text-white border border-white/10">
          <i class="fas fa-film text-[12px] opacity-80"></i> <?php echo e(__('public.courses_page_title')); ?>
        ...
        <h1 class="text-[2rem] sm:text-[2.8rem] lg:text-[3.35rem] leading-[1.18] font-black text-white mb-5 font-display">
          ...
          <span class="block text-acad-yellow"><?php echo e(__('public.browse_courses')); ?></span>
        </h1>
        <p class="text-white/70 text-base sm:text-lg leading-8 mb-7 max-w-3xl mx-auto"><?php echo e(__('public.courses_subtitle')); ?></p>
        ...
        <div class="grid sm:grid-cols-[1fr_180px] gap-3 max-w-3xl mx-auto">
          <div class="flex items-center gap-3 rounded-2xl glass-panel px-4 py-3 border border-white/12">
            ...
            <input type="text" x-model="searchQuery" placeholder="<?php echo e(__('public.search_course_placeholder')); ?>" class="flex-1 bg-transparent border-0 outline-none text-white placeholder:text-white/45 font-semibold">
          </div>
          <div class="relative">
            <select x-model="selectedCategoryId" class="w-full rounded-2xl glass-panel px-4 py-3 <?php echo e($isRtl?'pl-10':'pr-10'); ?> text-white focus:outline-none border border-white/12">
            ...
            <i class="fas fa-chevron-down absolute <?php echo e($isRtl?'left':'right'); ?>-3 top-1/2 -translate-y-1/2 text-white/50 text-xs"></i>
          </div>
        </div>

        <div class="flex flex-wrap items-center justify-center gap-2 mt-6 reveal s1">
          <a href="<?php echo e(route('public.courses')); ?>"
             class="px-4 py-2 rounded-full text-sm font-extrabold border transition <?php echo e(!$activeDelivery ? 'bg-acad-yellow text-[#0B3D91] border-acad-yellow' : 'glass-panel border-white/12 text-white/80 hover:text-white'); ?>">
            <?php echo e(__('public.courses_filter_all')); ?>

          </a>
          <a href="<?php echo e(route('public.courses', ['delivery' => 'group'])); ?>"
             class="px-4 py-2 rounded-full text-sm font-extrabold border transition <?php echo e($activeDelivery === 'group' ? 'bg-acad-cyan text-[#0B3D91] border-acad-cyan' : 'glass-panel border-white/12 text-white/80 hover:text-white'); ?>">
            <i class="fas fa-users text-xs <?php echo e($isRtl ? 'ml-1' : 'mr-1'); ?>"></i><?php echo e(__('public.courses_filter_group')); ?>

          </a>
          <a href="<?php echo e(route('public.courses', ['delivery' => 'one_to_one'])); ?>"
             class="px-4 py-2 rounded-full text-sm font-extrabold border transition <?php echo e($activeDelivery === 'one_to_one' ? 'bg-violet-400 text-[#0B3D91] border-violet-400' : 'glass-panel border-white/12 text-white/80 hover:text-white'); ?>">
            <i class="fas fa-user-graduate text-xs <?php echo e($isRtl ? 'ml-1' : 'mr-1'); ?>"></i><?php echo e(__('public.courses_filter_one_to_one')); ?>

            <?php if(($oneToOneCount ?? 0) > 0): ?>
              <span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full text-[10px] bg-white/15 <?php echo e($isRtl ? 'mr-1' : 'ml-1'); ?>"><?php echo e($oneToOneCount); ?></span>
            <?php endif; ?>
          </a>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mt-10 max-w-2xl mx-auto reveal s2">
        <article class="rounded-2xl p-4 sm:p-5 border border-white/10 glass-panel text-center">
          ...
          <p class="text-xs sm:text-sm text-white/55 mt-1"><?php echo e(__('public.courses_stats_available')); ?></p>
        </article>
        <article class="rounded-2xl p-4 sm:p-5 border border-white/10 glass-panel text-center">
          <p class="text-3xl sm:text-4xl font-black text-acad-yellow" x-text="courses.filter(c=>c.is_featured).length">0</p>
          <p class="text-xs sm:text-sm text-white/55 mt-1"><?php echo e(__('public.courses_stats_featured')); ?></p>
        </article>
        <article class="rounded-2xl p-4 sm:p-5 border border-white/10 glass-panel text-center hidden sm:block">
          ...
          <p class="text-xs sm:text-sm text-white/55 mt-1"><?php echo e(__('public.courses_filter_one_to_one')); ?></p>
        </article>
      </div>
    </div>
  </section>
    ...
    <div class="container-acad">
      <div class="flex items-end justify-between mb-7 gap-4">
        <div class="reveal max-w-2xl">
          <h2 class="text-3xl sm:text-4xl font-black text-white mb-2 font-display"><?php echo e(__('public.courses_section_title')); ?></h2>
          <p class="text-white/60"><?php echo e(__('public.courses_section_subtitle')); ?></p>
        </div>
      </div>
      ...
      <div x-data="{
            get rows() {
              ...
              if (q) return [{ title: '<?php echo e(addslashes(__('landing.academy.search_results_row'))); ?>', items: list }];
              const map = new Map();
              list.forEach(c => {
                const title = (c.course_category && c.course_category.name) || (c.academic_subject && c.academic_subject.name) || '<?php echo e(addslashes(__('public.all_course_categories'))); ?>';
                ...
                <div class="netflix-row" dir="<?php echo e($isRtl ? 'rtl' : 'ltr'); ?>">
                  <template x-for="course in row.items" :key="course.id">
                    <a class="netflix-item stream-card block"
                       :href="'<?php echo e(url('/course')); ?>/' + course.id">
                      <div class="relative aspect-video bg-slate-900/50">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition duration-300 bg-black/50">
                          <span class="px-5 py-2.5 rounded-full bg-acad-yellow text-[#0B3D91] font-black text-sm shadow-lg">
                            <i class="fas fa-play text-xs me-1"></i> <?php echo e(__('landing.academy.stream_play')); ?>

                          </span>
                        </div>
                        <template x-if="course.is_featured">
                          <span class="absolute top-3 <?php echo e($isRtl?'right':'left'); ?>-3 text-[10px] font-black px-2 py-1 rounded-md bg-acad-yellow text-[#0B3D91]"><?php echo e(__('public.featured_badge')); ?></span>
                        </template>
                        <template x-if="course.is_one_to_one">
                          <span class="absolute top-3 <?php echo e($isRtl?'left':'right'); ?>-3 text-[10px] font-black px-2 py-1 rounded-md bg-violet-400 text-[#0B3D91]"><?php echo e(__('public.course_badge_one_to_one')); ?></span>
                        </template>
                      </div>
                      <div class="p-4 text-start">
                        ...
                        <h4 class="font-black text-white text-sm sm:text-base leading-snug line-clamp-2" x-text="course.title || '<?php echo e(addslashes(__('public.no_title_fallback'))); ?>'"></h4>
                        <div class="mt-2 flex items-center justify-between text-xs font-bold text-white/55">
                          <span x-text="(course.lectures_count || 0) + ' <?php echo e(addslashes(__('public.lecture_single'))); ?>'"></span>
                          <span class="text-acad-cyan"><?php echo e(__('public.view_details')); ?></span>
                        </div>
                      </div>
                    </a>
                  </template>
      ...
      <div x-show="filteredCourses && filteredCourses.length === 0" x-cloak class="text-center py-16 reveal">
        <h3 class="text-2xl font-black text-white mb-2 font-display"><?php echo e(__('public.no_results')); ?></h3>
        <p class="text-white/60 mb-6"><?php echo e(__('public.no_results_hint')); ?></p>
        <button @click="searchQuery=''; selectedCategoryId='';" type="button" class="btn-secondary"><?php echo e(__('public.courses_reset_search')); ?></button>
      </div>
    </div>
  </section>
    ...
    <div class="container-acad">
      <div class="reveal rounded-[28px] border border-white/10 glass-panel px-6 sm:px-10 py-10 sm:py-12 text-center overflow-hidden relative">
        ...
        <span class="relative z-10 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs sm:text-sm font-extrabold mb-5 glass-panel border border-white/10">
          <i class="fas fa-rocket"></i> <?php echo e(__('public.courses_cta_badge')); ?>

        </span>
        <h2 class="relative z-10 text-3xl sm:text-5xl font-black text-white mb-4 font-display"><?php echo e(__('public.courses_cta_title')); ?></h2>
        <p class="relative z-10 text-white/70 text-base sm:text-lg max-w-3xl mx-auto leading-8 mb-7"><?php echo e(__('public.courses_cta_desc')); ?></p>
