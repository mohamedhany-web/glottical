...
<head>
  ...
  <link rel="preload" as="image" href="<?php echo e($authBg); ?>" fetchpriority="high">
  ...
  <style>
    ...
    .auth-brand-photo{
      background-image:url('<?php echo e($authBgCss); ?>');
...
<body class="auth-glottical font-sans antialiased text-ink" <?php echo $__env->yieldContent('body_attrs'); ?>>
  <div class="auth-shell min-h-screen lg:grid lg:grid-cols-2">
    ...
    <aside class="auth-brand-panel relative hidden overflow-hidden text-white lg:flex lg:flex-col lg:justify-between lg:p-12 xl:p-14">
      ...
      <img
        ...
        data-auth-bg
        onerror="this.onerror=null;this.src='<?php echo e($authBgFallback); ?>';var p=this.previousElementSibling;if(p){p.style.backgroundImage='url(\'<?php echo e($authBgFallback); ?>\')';}"
      >
      <div class="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink via-ink/80 to-ink/45"></div>
      <div class="pointer-events-none absolute -<?php echo e($isRtl ? 'left' : 'right'); ?>-24 top-1/3 size-72 rounded-full bg-accent/20 blur-3xl"></div>
      <div class="pointer-events-none absolute -<?php echo e($isRtl ? 'right' : 'left'); ?>-16 bottom-0 size-64 rounded-full bg-metal/15 blur-3xl"></div>

      <div class="relative z-10 fade-up">
        <?php echo $__env->make('partials.auth-brand-link', ['size' => 'sm', 'variant' => 'dark', 'fallback' => 'accent', 'mb' => 'mb-0'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
      </div>

      <div class="relative z-10 max-w-md space-y-8 fade-up fade-up-delay-1">
        <div class="space-y-4">
          <p class="text-sm font-medium text-metal"><?php echo $__env->yieldContent('brand_kicker', __('auth.visual_title')); ?></p>
          <h2 class="text-balance text-3xl font-semibold leading-tight xl:text-4xl"><?php echo $__env->yieldContent('brand_title', __('auth.visual_desc')); ?></h2>
          <p class="text-sm leading-8 text-white/65"><?php echo $__env->yieldContent('brand_lead'); ?></p>
        </div>
        ...
        <ul class="space-y-4">
          ...
          ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $point): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <li class="flex gap-3 fade-up fade-up-delay-<?php echo e(min($i + 2, 3)); ?>">
              ...
              <div>
                <p class="text-sm font-semibold text-white"><?php echo e($point['title']); ?></p>
                <p class="mt-0.5 text-xs leading-6 text-white/55"><?php echo e($point['desc']); ?></p>
              </div>
            </li>
    ...
    <div class="auth-form-col relative flex min-h-screen flex-col">
      ...
      <div class="auth-mobile-strip relative overflow-hidden bg-ink text-white lg:hidden">
        ...
        <div class="relative z-10 flex items-center justify-between gap-3 px-4 py-3.5 sm:px-5 sm:py-4">
          ...
          <div class="flex shrink-0 items-center gap-0.5 rounded-xl border border-white/15 bg-white/5 p-1 text-xs font-semibold backdrop-blur-sm">
            <a href="<?php echo e($langSwitch('ar')); ?>" class="min-h-8 rounded-lg px-2.5 py-1.5 transition <?php echo e($isRtl ? 'bg-white/20 text-white' : 'text-white/70 hover:text-white'); ?>" hreflang="ar">عربي</a>
            <a href="<?php echo e($langSwitch('en')); ?>" class="min-h-8 rounded-lg px-2.5 py-1.5 transition <?php echo e(! $isRtl ? 'bg-white/20 text-white' : 'text-white/70 hover:text-white'); ?>" hreflang="en">EN</a>
          </div>
        </div>
      </div>
      ...
      <header class="auth-desktop-header hidden items-center justify-between gap-3 px-5 py-4 sm:px-8 lg:flex lg:px-10">
        <div class="ms-auto flex flex-wrap items-center justify-end gap-2">
          <div class="flex items-center gap-0.5 rounded-xl border border-line bg-surface p-1 text-xs font-semibold shadow-soft">
            <a href="<?php echo e($langSwitch('ar')); ?>" class="rounded-lg px-3 py-1.5 transition <?php echo e($isRtl ? 'bg-accent-soft text-accent' : 'text-muted hover:text-ink'); ?>" hreflang="ar">عربي</a>
            <a href="<?php echo e($langSwitch('en')); ?>" class="rounded-lg px-3 py-1.5 transition <?php echo e(! $isRtl ? 'bg-accent-soft text-accent' : 'text-muted hover:text-ink'); ?>" hreflang="en">EN</a>
          </div>
          <a href="<?php echo e(route('home')); ?>" class="inline-flex h-9 items-center rounded-xl border border-line bg-surface px-3 text-xs font-medium text-muted shadow-soft transition hover:border-accent hover:text-accent">
      ...
      <main class="auth-form-main page-enter flex flex-1 flex-col justify-center px-4 py-6 sm:px-8 sm:py-8 lg:px-12 lg:py-10 xl:px-16">
        <div class="auth-form-card mx-auto w-full <?php echo $__env->yieldContent('form_max', 'max-w-[26rem]'); ?>">
      ...
      <footer class="auth-mobile-footer px-4 pb-5 text-center text-xs text-muted sm:px-8 lg:hidden" style="padding-bottom:max(1.25rem, env(safe-area-inset-bottom))">
        <a href="<?php echo e(route('home')); ?>" class="inline-flex min-h-10 items-center font-medium text-accent transition hover:text-ink"><?php echo e(__('auth.back_to_home')); ?></a>
      </footer>
    </div>
  </div>
...
<?php /**PATH C:\xampp\htdocs\glottical\resources\views\layouts\auth-atheer.blade.php ENDPATH**/ ?>