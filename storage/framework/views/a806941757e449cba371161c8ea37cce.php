...
<nav class="flex flex-wrap gap-2 rounded-2xl border bg-white p-3 text-sm">
    ...
    <?php if(\App\Services\Crm\CrmAccessService::hasCrmPermission($user, 'crm_create_leads') || $crmRole === 'marketing'): ?>
        <a href="<?php echo e(route('employee.crm.marketing.desk')); ?>" class="px-3 py-1.5 rounded-lg font-bold <?php echo e(request()->routeIs('employee.crm.marketing.*') ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-700'); ?>">مسوق بالعمولة</a>
    <?php endif; ?>

    <?php if(in_array($crmRole, ['sales', 'team_leader', 'super_admin'], true)): ?>
        <a href="<?php echo e(route('employee.crm.marketing-inbox.index')); ?>" class="px-3 py-1.5 rounded-lg font-bold <?php echo e(request()->routeIs('employee.crm.marketing-inbox.*') ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700'); ?>">بيانات المسوقين</a>
    <?php endif; ?>
