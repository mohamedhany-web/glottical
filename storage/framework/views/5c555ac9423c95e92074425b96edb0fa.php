...
<head>
    ...
    <title>انضم إلى Glottical Classroom — <?php echo e($code); ?></title>
...
<body>
    <div id="join-screen" class="min-h-screen flex flex-col items-center justify-center p-4">
        <div class="w-full max-w-md join-card overflow-hidden">
            <div class="join-hero-bar px-6 py-5 text-center">
                ...
                <h1 class="text-xl font-black m-0"><?php echo e(config('app.name')); ?> Classroom</h1>
            ...
            <?php if(!empty($meetingEnded)): ?>
                <div class="text-center">
                    ...
                    <p class="text-slate-400 text-xs mt-4">كود الغرفة: <span class="font-mono"><?php echo e($code); ?></span></p>
                </div>
            <?php elseif(!empty($guestJoinBlocked)): ?>
                <div class="text-center">
                    ...
                    <a href="<?php echo e(route('login')); ?>" class="mt-5 inline-flex w-full items-center justify-center px-6 py-3 rounded-xl bg-[var(--st-brand)] hover:opacity-95 text-white font-bold transition-colors">
                        تسجيل الدخول والانضمام
                    </a>
                </div>
            <?php elseif(!empty($meetingNotStarted)): ?>
                <div class="text-center">
                    ...
                    <p class="text-slate-400 text-xs mb-4">كود الغرفة: <span class="font-mono"><?php echo e($code); ?></span></p>
            ...
            <?php else: ?>
                <?php if($meeting && $meeting->title): ?>
                    <p class="text-slate-700 text-sm mb-3 text-center font-bold"><?php echo e($meeting->title); ?></p>
                <?php endif; ?>
                <p class="text-slate-500 text-xs mb-2 text-center">كود الغرفة: <span class="font-mono font-bold text-[var(--st-blue)] text-lg"><?php echo e($code); ?></span></p>
                <p class="text-slate-500 text-xs mb-4 text-center">الحد الأقصى: <span class="font-bold text-amber-600"><?php echo e($maxParticipants); ?></span></p>
                <?php if(empty($livekitConfigured)): ?>
                ...
                <div class="mt-6">
                    <button type="button" id="btn-join" class="w-full px-6 py-3 rounded-xl bg-[var(--st-gold)] hover:brightness-95 text-[#072A66] font-black transition-colors" <?php echo e(empty($livekitConfigured) ? 'disabled' : ''); ?>>
    ...
    <div id="meeting-screen" class="hidden h-screen flex flex-col bg-slate-950 text-white">
        <header class="h-[72px] join-hero-bar flex items-center justify-between px-4 sm:px-6 shadow-lg flex-shrink-0 gap-2">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <span class="font-bold text-white truncate"><?php echo e(config('app.name')); ?> Classroom</span>
                <span class="text-white/75 text-sm shrink-0">— <?php echo e($code); ?></span>
            </div>
    ...
    <?php if(empty($meetingEnded) && empty($meetingNotStarted)): ?>
    <script>
        const code = <?php echo json_encode($code, 15, 512) ?>;
            ...
            try {
                ...
                if (window.MxClassroomCurriculumPresenter) {
                    ...
                    window.__mxCurriculumPresenter = window.MxClassroomCurriculumPresenter.attach(null, {
                        ...
                        stateUrl: <?php echo json_encode(route('classroom.join.curriculum.state', $code), 512) ?>,
                        pollIntervalMs: 1500,
                    });
                }
...
<?php /**PATH /Users/cityphone/Documents/glottical/resources/views/classroom/join.blade.php ENDPATH**/ ?>