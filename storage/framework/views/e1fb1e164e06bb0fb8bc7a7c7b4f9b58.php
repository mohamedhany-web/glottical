...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo e(route($leadsRoute)); ?>" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold shadow-sm transition-colors">
            ...
            <?php if(!empty($useCrm)): ?>
                <a href="<?php echo e(route('employee.crm.dashboard')); ?>" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold shadow-sm transition-colors">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div class="rounded-2xl border-2 border-violet-200/60 bg-gradient-to-br from-white to-violet-50/80 p-4 sm:p-5 shadow-sm">
            ...
            <p class="text-2xl sm:text-3xl font-black text-violet-800 tabular-nums"><?php echo e(number_format($stats['mine_won_month'] ?? 0)); ?></p>
        </div>
        <a href="<?php echo e(route($leadsRoute, array_filter(['open' => !empty($useCrm) ? 1 : null]))); ?>" class="rounded-2xl border-2 border-teal-200/60 bg-gradient-to-br from-white to-teal-50/80 p-4 sm:p-5 shadow-sm block hover:border-teal-300 transition-colors">
        ...
        <a href="<?php echo e(route($leadsRoute, array_filter(['mine' => 1, 'open' => !empty($useCrm) ? 1 : null]))); ?>" class="rounded-2xl border-2 border-cyan-200/60 bg-gradient-to-br from-white to-cyan-50/80 p-4 sm:p-5 shadow-sm block hover:border-cyan-300 transition-colors">
        ...
        <div class="rounded-2xl border-2 border-rose-200/60 bg-gradient-to-br from-white to-rose-50/80 p-4 sm:p-5 shadow-sm">
            ...
            <p class="text-2xl sm:text-3xl font-black text-rose-800 tabular-nums"><?php echo e(number_format($stats['rejected'] ?? 0)); ?></p>
        </div>
    </div>
                    ...
                    <li>
                        <a href="<?php echo e(route('employee.sales.orders.show', $o)); ?>" class="block px-5 py-3 hover:bg-gray-50 transition-colors">
                            <div class="flex justify-between gap-2">
                                <span class="font-medium text-gray-900">#<?php echo e($o->id); ?> — <?php echo e(\Illuminate\Support\Str::limit($o->course?->title ?? '—', 32)); ?></span>
    ...
    <div class="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-900">
        ...
        <?php if(!empty($useCrm)): ?>
            لمتابعة العملاء المحتملين وحالات البيع استخدم <a href="<?php echo e(route('employee.crm.dashboard')); ?>" class="font-bold underline">Glottical CRM</a>.
        <?php endif; ?>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.employee', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views\employee\sales\desk.blade.php ENDPATH**/ ?>