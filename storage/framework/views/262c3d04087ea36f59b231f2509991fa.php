...
<?php $__env->startSection('page_title', 'تفاصيل الفاتورة'); ?>
...
<div class="space-y-5 invoice-print-wrapper">
    <section class="flex flex-wrap items-end justify-between gap-4" data-print-hide>
        <div class="min-w-0">
            <p class="text-xs font-medium text-muted">المحاسبة · الفواتير · #<?php echo e($invoice->invoice_number); ?></p>
            <h2 class="mt-1 text-2xl font-semibold tracking-tight text-ink md:text-[28px]">فاتورة #<?php echo e($invoice->invoice_number); ?></h2>
            <p class="mt-1 text-sm text-muted">أُنشئت في <?php echo e($invoice->created_at->format('Y-m-d')); ?></p>
        </div>
        <div class="admin-hero-actions flex flex-wrap gap-2">
            <a href="<?php echo e(route('admin.invoices.edit', $invoice)); ?>" class="btn-press inline-flex h-9 items-center gap-2 rounded-xl border border-line bg-surface px-4 text-sm font-medium text-ink-soft transition hover:border-accent/30 hover:text-accent">
        ...
        <div id="invoice-print-area" class="space-y-5 p-4 sm:p-5 lg:p-6">
            <div class="rounded-2xl border border-line bg-surface p-5 print-card">
                <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h3 class="text-xl font-semibold text-ink"><?php echo e(config('app.name')); ?></h3>
                        <p class="mt-2 text-sm text-muted"><?php echo e(config('services.platform.support_email')); ?> · <?php echo e(config('services.platform.support_phone')); ?></p>
                    </div>
                    <div class="text-sm text-muted md:text-left">
                        <p><span class="text-muted">رقم الفاتورة:</span> <span class="font-semibold text-ink">#<?php echo e($invoice->invoice_number); ?></span></p>
                        <p><span class="text-muted">تاريخ الإنشاء:</span> <span class="font-semibold text-ink"><?php echo e($invoice->created_at->format('Y-m-d')); ?></span></p>
                        <p><span class="text-muted">تاريخ الطباعة:</span> <span class="font-semibold text-ink"><?php echo e(now()->format('Y-m-d H:i')); ?></span></p>
                    </div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="rounded-2xl border border-line bg-canvas-muted/40 p-5 text-sm text-ink-soft print-strip">
                        ...
                        <dl class="mt-3 space-y-2">
                            <div class="flex justify-between gap-6"><dt class="text-muted">الاسم</dt><dd class="font-semibold text-ink"><?php echo e($invoice->user->name ?? 'غير معروف'); ?></dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">رقم الهاتف</dt><dd class="font-semibold text-ink"><?php echo e($invoice->user->phone ?? '—'); ?></dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">البريد الإلكتروني</dt><dd class="font-semibold text-ink"><?php echo e($invoice->user->email ?? '—'); ?></dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">نوع الفاتورة</dt><dd class="font-semibold text-ink"><?php echo e($invoice->type); ?></dd></div>
                        </dl>
                    </div>
                    <div class="rounded-2xl border border-line bg-canvas-muted/40 p-5 text-sm text-ink-soft print-strip">
                        ...
                        <dl class="mt-3 space-y-2">
                            <div class="flex justify-between gap-6"><dt class="text-muted">الحالة</dt><dd>
                                <span class="inline-flex items-center gap-2 rounded-lg px-2.5 py-1 text-xs font-medium status-badge <?php echo e($badge['classes']); ?>" data-status="<?php echo e($invoice->status); ?>">
                                    <span class="h-1.5 w-1.5 rounded-full bg-current"></span>
                                    <?php echo e($badge['label']); ?>

                                </span>
                            </dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">تاريخ الاستحقاق</dt><dd class="font-semibold text-ink"><?php echo e($invoice->due_date ? $invoice->due_date->format('Y-m-d') : '—'); ?></dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">آخر تحديث</dt><dd class="font-semibold text-ink"><?php echo e($invoice->updated_at->format('Y-m-d H:i')); ?></dd></div>
                            <div class="flex justify-between gap-6"><dt class="text-muted">ملاحظات</dt><dd class="font-semibold text-ink"><?php echo e($invoice->notes ?: '—'); ?></dd></div>
                        </dl>
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-sm text-ink-soft">
                        <tbody class="divide-y divide-line">
                            <tr class="flex items-center justify-between py-3">
                                <td>المبلغ الفرعي</td>
                                <td class="font-semibold tabular-nums text-ink"><?php echo e(number_format($invoice->subtotal, 2)); ?> ج.م</td>
                            </tr>
                            <?php if($invoice->tax_amount > 0): ?>
                                <tr class="flex items-center justify-between py-3">
                                    <td>الضريبة</td>
                                    <td class="font-semibold tabular-nums text-ink"><?php echo e(number_format($invoice->tax_amount, 2)); ?> ج.م</td>
                                </tr>
                            <?php endif; ?>
                            <?php if($invoice->discount_amount > 0): ?>
                                <tr class="flex items-center justify-between py-3">
                                    <td>الخصم</td>
                                    <td class="font-semibold tabular-nums text-metal">-<?php echo e(number_format($invoice->discount_amount, 2)); ?> ج.م</td>
                                </tr>
                            <?php endif; ?>
                            <tr class="flex items-center justify-between py-3 text-base font-semibold text-ink">
                                <td>الإجمالي المستحق</td>
                                <td class="tabular-nums text-accent"><?php echo e(number_format($invoice->total_amount, 2)); ?> ج.م</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                            ...
                            <tbody class="divide-y divide-line">
                                <?php $__currentLoopData = $invoice->payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-canvas/40">
                                        <td class="px-4 py-3 font-semibold text-ink"><?php echo e($payment->payment_number); ?></td>
                                        <td class="px-4 py-3 text-center text-muted"><?php echo e($payment->paid_at ? $payment->paid_at->format('Y-m-d') : '—'); ?></td>
                                        <td class="px-4 py-3 text-end font-semibold tabular-nums text-ink"><?php echo e(number_format($payment->amount, 2)); ?> ج.م</td>
                                    </tr>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views\admin\invoices\show.blade.php ENDPATH**/ ?>