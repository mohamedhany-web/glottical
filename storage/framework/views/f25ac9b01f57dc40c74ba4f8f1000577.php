...
<?php $__env->startSection('page_title', 'اتفاقيات التقسيط'); ?>
...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="admin-hero-actions flex flex-wrap gap-2">
            ...
            <a href="<?php echo e(route('admin.installments.agreements.create')); ?>" class="btn-press inline-flex h-9 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-medium text-white">
    ...
    <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
        ...
        <form method="GET" class="flex flex-wrap items-end gap-4 p-4 sm:p-5">
            ...
            <?php if(request()->filled('status')): ?>
                <a href="<?php echo e(route('admin.installments.agreements.index')); ?>" class="btn-press inline-flex h-11 items-center gap-2 rounded-xl border border-line px-5 text-sm font-medium text-ink hover:bg-canvas">
    ...
    <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-4 sm:px-5">
            ...
            <span class="inline-flex rounded-lg bg-accent-soft px-2.5 py-1 text-xs font-medium text-accent"><?php echo e(number_format($total)); ?> اتفاقية</span>
        </div>
        ...
        <?php if($agreements->count()): ?>
                ...
                <?php $__currentLoopData = $agreements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $agreement): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div class="flex flex-col gap-4 rounded-2xl border border-line bg-canvas/30 p-5">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <p class="text-sm font-semibold text-ink"><?php echo e($agreement->student->name ?? 'طالب غير معروف'); ?></p>
                                <p class="mt-1 text-xs text-accent"><?php echo e($agreement->course->title ?? 'خطة عامة'); ?></p>
                                <p class="mt-0.5 text-[11px] text-muted">بداية <?php echo e(optional($agreement->start_date)->format('Y-m-d')); ?></p>
                            </div>
                            <span class="inline-flex shrink-0 rounded-lg px-2.5 py-1 text-xs font-medium <?php echo e($badgeClasses); ?>">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <p class="mt-1 font-semibold tabular-nums text-ink"><?php echo e(number_format($agreement->total_amount ?? 0, 2)); ?> <span class="text-xs font-normal text-muted">ج.م</span></p>
                            </div>
                            <div>
                                <p class="text-[11px] font-medium uppercase text-muted">دفعة مقدمة</p>
                                <p class="mt-1 font-semibold tabular-nums text-ink"><?php echo e(number_format($agreement->deposit_amount ?? 0, 2)); ?> <span class="text-xs font-normal text-muted">ج.م</span></p>
                            </div>
                            <div>
                                <p class="text-[11px] font-medium uppercase text-muted">عدد الأقساط</p>
                                <p class="mt-1 font-medium text-ink"><?php echo e($agreement->installments_count); ?> دفعة</p>
                            </div>
                            <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <a href="<?php echo e(route('admin.installments.agreements.show', $agreement)); ?>" class="btn-press inline-flex h-8 flex-1 items-center justify-center gap-2 rounded-lg bg-accent px-3 text-xs font-medium text-white">
                                <i class="fas fa-eye"></i>
                                عرض التفاصيل
                            </a>
                            <a href="<?php echo e(route('admin.installments.agreements.edit', $agreement)); ?>" class="btn-press inline-flex size-8 items-center justify-center rounded-lg border border-line text-ink transition hover:border-accent/30 hover:text-accent" title="تعديل">
            ...
            <?php if($agreements instanceof \Illuminate\Pagination\LengthAwarePaginator && $agreements->hasPages()): ?>
                <div class="border-t border-line px-4 py-4 sm:px-5"><?php echo e($agreements->withQueryString()->links()); ?></div>
            <?php endif; ?>
        <?php else: ?>
            <div class="px-4 py-16 text-center sm:px-5">
                ...
                <a href="<?php echo e(route('admin.installments.agreements.create')); ?>" class="btn-press mt-4 inline-flex h-9 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-medium text-white">
                    <i class="fas fa-plus text-xs"></i>
                    إنشاء اتفاقية جديدة
                </a>
            </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\glottical\resources\views\admin\installments\agreements\index.blade.php ENDPATH**/ ?>