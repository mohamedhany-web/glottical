...
<head>
    ...
    <title>Glottical Classroom — <?php echo e($meeting->roomChromeTitle()); ?></title>
...
?>
    ...
    <header class="min-h-14 shrink-0 bg-gradient-to-l from-slate-900 to-slate-800 border-b border-slate-700/50 flex flex-col gap-2 px-3 sm:px-4 pt-[max(0.5rem,env(safe-area-inset-top))] pb-2 md:py-0 md:flex-row md:items-center md:justify-between md:gap-2 shadow-lg">
        <div class="flex items-center justify-between gap-2 w-full min-w-0 md:w-auto md:flex-1 md:justify-start">
            ...
            <div class="flex items-center gap-1.5 min-w-0">
                ...
                <span class="text-white font-semibold text-xs sm:text-sm truncate"><?php echo e($meeting->roomChromeTitle()); ?></span>
        ...
        <div id="mx-classroom-toolbar-inner" class="flex w-full flex-col items-stretch gap-3 md:w-auto md:min-w-max md:flex-row md:flex-nowrap md:items-center md:justify-end md:gap-1 md:gap-2 md:max-w-[min(100%,42rem)] lg:max-w-none pe-1 ps-0.5">
            ...
            <span class="hidden sm:inline-flex text-amber-200 text-[10px] sm:text-[11px] px-1.5 py-0.5 rounded-md bg-amber-500/20 border border-amber-500/30 whitespace-nowrap" id="meeting-timer-chip">
                مدة الاجتماع: <?php echo e((int) $effectiveDurationMinutes); ?> دقيقة (الحد <?php echo e((int) $maxDurationMinutes); ?>)
            </span>
            ...
            <?php endif; ?>
            <form method="POST" action="<?php echo e($mxRoute($rp.'classroom.end', $meeting)); ?>" class="inline w-full shrink-0 md:w-auto <?php echo e(!\Illuminate\Support\Facades\Route::has($rp.'classroom.end') ? 'hidden' : ''); ?>" id="mx-end-meeting-form" onsubmit="return (typeof window.__mxClassroomConfirmEnd === 'function') ? window.__mxClassroomConfirmEnd() : confirm('إنهاء الاجتماع للجميع؟');">
                <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
            <a href="<?php echo e($roomExitUrl ?? route('dashboard')); ?>" class="classroom-room-toolbar-btn w-full justify-center gap-2 bg-slate-700/80 hover:bg-slate-600 text-slate-100 border border-slate-600 md:w-auto md:justify-start" title="العودة للوحة">
    ...
    <script>
        (function() {
            ...
            var mxSilentAutoRecording = <?php echo e(!empty($canManageMeeting) ? 'true' : 'false'); ?>;
            ...
            var uploadRecordingUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording.upload', $meeting), 512) ?>;
            var presignRecordingUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording.presign', $meeting), 512) ?>;
            var completeRecordingUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording.complete', $meeting), 512) ?>;
            var presignAudioUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording-audio.presign', $meeting), 512) ?>;
            var uploadAudioUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording-audio.upload', $meeting), 512) ?>;
            var completeAudioUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording-audio.complete', $meeting), 512) ?>;
            var recordingUploadTabBaseUrl = <?php echo json_encode($mxRoute($rp . 'classroom.recording.upload-tab', $meeting), 512) ?>;
            var csrfToken = '<?php echo e(csrf_token()); ?>';
            var participantWbUrl = <?php echo json_encode($mxRoute($rp . 'classroom.participant-whiteboard', $meeting), 512) ?>;
                                            ...
                                            (function bindWbSync(tries) {
                                                try {
                                                    ...
                                                    window.__mxClassroomWbSync = window.MxClassroomWhiteboardSync.attach({
                                                        ...
                                                        canEmit: true,
                                                        canReceive: true,
                                                        mergeRemote: <?php echo e(!empty($meeting->allowsParticipantWhiteboard()) ? 'true' : 'false'); ?>,
                                                        stateUrl: <?php echo json_encode($mxRoute($rp . 'classroom.whiteboard.state', $meeting), 512) ?>,
                                                        pushUrl: <?php echo json_encode($mxRoute($rp . 'classroom.whiteboard.push', $meeting), 512) ?>,
                                                        csrf: csrfToken,
                                                    });
                                                } catch (eWb) {}
                                            })(0);
            ...
            function attachCurriculumPresenter() {
                ...
                window.__mxCurriculumPresenter = window.MxClassroomCurriculumPresenter.attach(null, {
                    isHost: <?php echo e(!empty($canManageMeeting) ? 'true' : 'false'); ?>,
                    catalogUrl: <?php echo json_encode($mxRoute($rp . 'classroom.curriculum.catalog', $meeting), 512) ?>,
                    presentUrl: <?php echo json_encode($mxRoute($rp . 'classroom.curriculum.present', $meeting), 512) ?>,
                    stateUrl: <?php echo json_encode($mxRoute($rp . 'classroom.curriculum.state', $meeting), 512) ?>,
                    slideUpdateUrl: <?php echo json_encode($mxRoute($rp . 'classroom.curriculum.slide.update', $meeting), 512) ?>,
                    stopUrl: <?php echo json_encode($mxRoute($rp . 'classroom.curriculum.stop', $meeting), 512) ?>,
                    pollIntervalMs: 1500,
                });
            }
...
<?php /**PATH /Users/cityphone/Documents/glottical/resources/views/student/classroom/room.blade.php ENDPATH**/ ?>