...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <form action="{{ route('student.classroom.store') }}" method="POST" class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm p-6 space-y-5">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', 'غرفة Glottical - ' . now()->format('H:i')) }}" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white">
