...
@section('title', __('public.help_page_title') . ' — ' . $brand)
...
<section class="-mt-14 sm:-mt-[60px] pt-24 sm:pt-28 lg:pt-32 pb-12 sm:pb-14 overflow-hidden relative">
    ...
    <div class="container-acad relative z-10 text-center">
        <span class="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs sm:text-sm font-extrabold mb-6 glass-panel border border-white/10 text-white">
            <i class="fas fa-life-ring text-acad-cyan"></i> {{ __('public.help_hero_badge') }}
        </span>
        <h1 class="text-[1.85rem] sm:text-[2.6rem] lg:text-[3.15rem] leading-[1.12] font-black mb-5 text-white font-display">
            ...
            <span class="block mt-2 text-acad-yellow text-[1.5rem] sm:text-[2rem] lg:text-[2.35rem]">{{ $brand }}</span>
        </h1>
        <p class="text-white/70 text-base sm:text-lg leading-8 max-w-2xl mx-auto mb-8">
            {{ __('public.help_hero_sub') }}
        </p>
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <a href="{{ route('public.faq') }}" class="btn-stream-primary px-7 py-3.5">
                <i class="fas fa-circle-question"></i> {{ __('public.faq_page_title') }}
            </a>
            <a href="{{ route('public.contact') }}" class="btn-stream-secondary px-7 py-3.5">
...
<section class="py-12 sm:py-16 relative">
    ...
    <div class="container-acad relative z-10">
        <div class="flex items-center gap-3 mb-8 justify-center sm:justify-start">
            ...
            <h2 class="text-xl sm:text-2xl font-black text-white font-display">{{ __('public.help_start_title') }}</h2>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6">
            <a href="{{ route('public.faq') }}" class="help-hub-card rounded-[24px] p-6 sm:p-7 flex flex-col items-center text-center no-underline text-inherit">
                ...
                <h3 class="text-lg font-black text-white mb-2">{{ __('public.help_card_faq_title') }}</h3>
                <p class="text-white/65 text-sm leading-relaxed">{{ __('public.help_card_faq_desc') }}</p>
            </a>
            <a href="{{ route('public.contact') }}" class="help-hub-card rounded-[24px] p-6 sm:p-7 flex flex-col items-center text-center no-underline text-inherit">
                ...
                <h3 class="text-lg font-black text-white mb-2">{{ __('public.help_card_contact_title') }}</h3>
                <p class="text-white/65 text-sm leading-relaxed">{{ __('public.help_card_contact_desc') }}</p>
            </a>
            <a href="{{ route('public.courses') }}" class="help-hub-card rounded-[24px] p-6 sm:p-7 flex flex-col items-center text-center no-underline text-inherit">
                ...
                <h3 class="text-lg font-black text-white mb-2">{{ __('public.help_card_courses_title') }}</h3>
                <p class="text-white/65 text-sm leading-relaxed">{{ __('public.help_card_courses_desc') }}</p>
            </a>
        </div>
        ...
        <div class="flex items-center gap-3 mt-14 mb-6 justify-center sm:justify-start">
            ...
            <h2 class="text-xl sm:text-2xl font-black text-white font-display">{{ __('public.help_topics_title') }}</h2>
        </div>
        <div class="rounded-[28px] border border-white/10 glass-panel overflow-hidden">
            ...
            <a href="{{ $topicLinks[$i - 1] }}" class="help-topic-row block px-5 sm:px-8 py-5 sm:py-6 no-underline text-inherit">
                <div class="flex items-start gap-4">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-acad-navy text-base shadow-sm font-black {{ $i % 2 === 0 ? 'bg-acad-cyan' : 'bg-acad-yellow' }}">
                        <i class="fas fa-{{ $topicIcons[$i - 1] }}"></i>
                    </div>
                    <div class="flex-1 min-w-0 text-start">
                        <h3 class="text-base sm:text-lg font-black text-white mb-1">{{ __('public.help_topic_'.$i.'_title') }}</h3>
                        <p class="text-white/60 text-sm leading-relaxed">{{ __('public.help_topic_'.$i.'_desc') }}</p>
                    </div>
                    <i class="fas {{ $chevronNext }} text-white/35 flex-shrink-0 mt-2 text-sm"></i>
                </div>
            </a>
            @endforeach
        </div>
        ...
        <div class="flex items-center gap-3 mt-14 mb-8 justify-center sm:justify-start">
            <span class="w-12 h-1 rounded-full shrink-0 bg-gradient-to-r from-acad-yellow to-acad-cyan"></span>
            <h2 class="text-xl sm:text-2xl font-black text-white font-display">{{ __('public.help_steps_title') }}</h2>
        </div>
        <div class="relative max-w-3xl mx-auto ps-8 sm:ps-10 border-s-2 border-white/20">
            @foreach(range(1, 4) as $step)
            <div class="relative pb-10 last:pb-0">
                <span class="absolute top-0 flex items-center justify-center w-9 h-9 rounded-full text-acad-navy text-sm font-black -start-[41px] sm:-start-[45px] {{ $step % 2 === 1 ? 'bg-acad-yellow' : 'bg-acad-cyan' }}">
                    {{ $step }}
                </span>
                <div class="rounded-2xl border border-white/10 glass-panel px-5 py-4">
                    <p class="font-black text-white mb-1">{{ __('public.help_step_'.$step.'_title') }}</p>
                    <p class="text-sm text-white/65">{{ __('public.help_step_'.$step.'_desc') }}</p>
                </div>
            </div>
            @endforeach
        </div>
...
<section class="pt-4 sm:pt-6 pb-14 sm:pb-16 relative">
    ...
    <div class="container-acad relative z-10">
        <div class="rounded-[28px] border border-white/10 glass-panel px-6 sm:px-10 py-10 sm:py-12 text-center overflow-hidden relative">
            ...
            <h3 class="relative z-10 text-2xl sm:text-3xl font-black mb-3 text-white font-display">{{ __('public.help_cta_title') }}</h3>
            ...
            <div class="relative z-10 flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
                ...
                <a href="{{ route('public.faq') }}" class="btn-stream-secondary px-8 py-3.5">
                    <i class="fas fa-circle-question"></i> {{ __('public.faq_page_title') }}
                </a>
            </div>
        </div>
