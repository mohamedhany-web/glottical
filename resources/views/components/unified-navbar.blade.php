            ...
            <div class="hidden lg:flex flex-1 min-w-0 items-center justify-center px-2 xl:px-4 relative z-10">
                <div class="flex items-center gap-1.5 xl:gap-2 max-w-full overflow-x-auto overflow-y-visible no-scrollbar py-1 -my-1 justify-center">
                    <div class="relative group/mega">
                        ...
                        <div class="mega-panel absolute top-full {{ $isRtl ? 'right-0' : 'left-1/2 -translate-x-1/2' }} mt-2 w-[min(720px,calc(100vw-2rem))] rounded-2xl shadow-2xl shadow-black/40 opacity-0 invisible group-hover/mega:opacity-100 group-hover/mega:visible group-focus-within/mega:opacity-100 group-focus-within/mega:visible translate-y-1 group-hover/mega:translate-y-0 group-focus-within/mega:translate-y-0 transition-all duration-200 z-[1000] p-4 text-start border border-white/12 bg-slate-900/92 backdrop-blur-xl">
                            <div class="grid sm:grid-cols-2 gap-2">
                                @foreach($megaCategories as $mc)
                                    <a href="{{ $mc['url'] }}" class="flex gap-3 rounded-xl p-3 transition hover:bg-white/8 border border-transparent hover:border-white/10 group/mc">
                                        @if(!empty($mc['thumb_url']))
                                            <span class="w-12 h-12 rounded-lg overflow-hidden shrink-0 ring-1 ring-white/15 bg-white/5">
                                                <img src="{{ $mc['thumb_url'] }}" alt="" width="48" height="48" class="w-full h-full object-cover transition duration-300 group-hover/mc:scale-105" loading="lazy" decoding="async">
                                            </span>
                                        @else
                                            <span class="w-12 h-12 rounded-lg flex items-center justify-center shrink-0 bg-acad-cyan/15 text-acad-cyan"><i class="fas {{ $mc['icon'] }}"></i></span>
                                        @endif
                                        <span class="min-w-0 flex-1">
                                            <span class="flex items-center gap-2">
                                                <span class="block font-extrabold text-sm text-white truncate">{{ $mc['name'] }}</span>
                                                @if(!empty($mc['count']))
                                                    <span class="shrink-0 text-[10px] font-black px-1.5 py-0.5 rounded-md bg-acad-yellow/15 text-acad-yellow border border-acad-yellow/25">{{ $mc['count'] }}</span>
                                                @endif
                                            </span>
                            ...
                            <a href="{{ route('public.courses', ['delivery' => 'one_to_one']) }}" class="mt-1 block text-center text-sm font-bold py-2 text-violet-300 hover:text-white">{{ __('public.nav_one_to_one_courses') }}</a>
                        </div>
                    </div>
