...
@section('title', 'المحاضرات - ' . config('app.name'))
...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
            ...
            <h2 class="mt-1 text-2xl font-semibold tracking-tight text-ink md:text-[28px]">المحاضرات</h2>
            <p class="mt-1 max-w-2xl text-sm text-muted">اختر برنامجاً لعرض محاضراته وإدارتها.</p>
        </div>
    </section>
                   ...
                   class="group rounded-2xl border border-line bg-surface p-4 shadow-soft transition hover:border-accent/30 hover:bg-accent-soft/30">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            @if($course->instructor)
                                <p class="mt-1 truncate text-xs text-muted">{{ $course->instructor->name }}</p>
                            @endif
                        </div>
                    ...
                    <div class="mt-4 flex items-center justify-between">
                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-[#f2f5f4] px-2.5 py-1 text-xs font-medium text-ink-soft">
                            <i class="fas fa-video text-[10px] text-accent"></i>
    ...
    @else
        <article class="rounded-2xl border border-dashed border-line bg-surface px-6 py-14 text-center shadow-soft">
            ...
            <p class="mt-1 text-sm text-muted">أضف برنامجاً أولاً ثم أنشئ محاضراته.</p>
        </article>
    @endif
</div>
@endsection
