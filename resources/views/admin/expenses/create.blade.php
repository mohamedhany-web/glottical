...
@section('page_title', 'إنشاء مصروف جديد')
...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
            ...
            <h2 class="mt-1 text-2xl font-semibold tracking-tight text-ink md:text-[28px]">{{ __('إنشاء مصروف جديد') }}</h2>
            <p class="mt-1 text-sm text-muted">{{ __('إضافة مصروف جديد للموافقة عليه') }}</p>
        </div>
        ...
        <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
            ...
            <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
                <div>
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title') }}" required class="{{ $fieldClass }}" placeholder="{{ __('مثال: شراء معدات للقاعة') }}">
                    @error('title')
                ...
                <div>
                    ...
                    <select id="category" name="category" required class="{{ $fieldClass }}">
                        ...
                        <option value="operational" @selected(old('category') == 'operational')>{{ __('تشغيلي') }}</option>
                        <option value="marketing" @selected(old('category') == 'marketing')>{{ __('تسويق') }}</option>
                        <option value="salaries" @selected(old('category') == 'salaries')>{{ __('رواتب') }}</option>
                        <option value="utilities" @selected(old('category') == 'utilities')>{{ __('مرافق') }}</option>
                        <option value="equipment" @selected(old('category') == 'equipment')>{{ __('معدات') }}</option>
                        <option value="maintenance" @selected(old('category') == 'maintenance')>{{ __('صيانة') }}</option>
                        <option value="other" @selected(old('category') == 'other')>{{ __('أخرى') }}</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-xs text-danger">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input id="amount" type="number" name="amount" step="0.01" min="0.01" value="{{ old('amount') }}" required class="{{ $fieldClass }}" placeholder="0.00">
                    @error('amount')
                        <p class="mt-1 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="expense_date" type="date" name="expense_date" value="{{ old('expense_date', date('Y-m-d')) }}" required class="{{ $fieldClass }}">
                    @error('expense_date')
                        <p class="mt-1 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
                <div>
                    ...
                    <select id="payment_method" name="payment_method" required class="{{ $fieldClass }}">
                        ...
                        <option value="cash" @selected(old('payment_method') == 'cash')>{{ __('نقدي') }}</option>
                        <option value="bank_transfer" @selected(old('payment_method') == 'bank_transfer')>{{ __('تحويل بنكي') }}</option>
                        <option value="card" @selected(old('payment_method') == 'card')>{{ __('بطاقة') }}</option>
                        <option value="wallet" @selected(old('payment_method') == 'wallet')>{{ __('محفظة إلكترونية') }}</option>
                        <option value="other" @selected(old('payment_method') == 'other')>{{ __('أخرى') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <input id="reference_number" type="text" name="reference_number" value="{{ old('reference_number') }}" class="{{ $fieldClass }}" placeholder="{{ __('رقم الفاتورة، رقم الشيك، إلخ') }}">
        ...
        <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft">
            ...
            <div class="space-y-4 p-4 sm:p-5">
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="{{ $areaClass }}" placeholder="{{ __('وصف تفصيلي للمصروف...') }}">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2" class="{{ $areaClass }}" placeholder="{{ __('ملاحظات إضافية...') }}">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-wrap gap-3 border-t border-line px-4 py-4 sm:px-5">
                ...
                <a href="{{ route('admin.expenses.index') }}" class="btn-press inline-flex h-11 items-center gap-2 rounded-xl border border-line px-6 text-sm font-medium text-ink hover:bg-canvas">
