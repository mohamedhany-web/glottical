...
@section('header', 'سلايدر الهيرو — الصفحة الرئيسية')
@section('content')
<div class="w-full space-y-6">
    <div class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 border-b border-slate-200 flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.homepage-sliders.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white rounded-xl font-semibold shadow-lg transition-all">
        ...
        <div class="p-5 sm:p-8">
            ...
            @if($rows->isEmpty())
                <div class="text-center py-16 px-4 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                    ...
                    <a href="{{ route('admin.homepage-sliders.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 text-white font-semibold hover:bg-amber-600">إضافة أول سلايدر</a>
                </div>
            @else
                <form method="GET" class="flex flex-wrap gap-3 mb-6">
                    <select name="source_type" class="px-4 py-2 border border-slate-200 rounded-xl text-sm">
                        <option value="">كل الأنواع</option>
                        <option value="course" @selected(request('source_type')==='course')>كورس</option>
                        <option value="path" @selected(request('source_type')==='path')>مسار</option>
                        <option value="custom" @selected(request('source_type')==='custom')>مخصص</option>
                    </select>
                ...
                <div id="slider-sortable" class="space-y-2">
                    ...
                    <div class="slider-row flex flex-wrap items-center gap-3 p-4 rounded-2xl border border-slate-200 bg-white hover:border-amber-200 transition cursor-grab active:cursor-grabbing" data-id="{{ $row->id }}">
                        ...
                        <div class="flex-1 min-w-[180px]">
                            <p class="font-bold text-slate-900">{{ $row->resolvedTitle() ?: '—' }}</p>
                            <p class="text-xs text-slate-500 mt-0.5">
                                <span class="rounded-full px-2 py-0.5 bg-slate-100 text-slate-700 font-semibold">{{ $row->sourceTypeLabel() }}</span>
                                @if($row->course) · {{ Str::limit($row->course->title, 40) }} @endif
                                @if($row->academicYear) · {{ Str::limit($row->academicYear->name, 40) }} @endif
                            </p>
                        </div>
                        <div class="text-sm text-slate-500">#{{ $row->sort_order }}</div>
                        <div>
                            @if($row->is_active)
                        ...
                        <div class="flex flex-wrap items-center gap-2 ms-auto">
                            <form action="{{ route('admin.homepage-sliders.toggle-active', $row) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50">{{ $row->is_active ? 'إيقاف' : 'تفعيل' }}</button>
                            </form>
                            <a href="{{ route('admin.homepage-sliders.edit', $row) }}" class="text-sky-600 hover:text-sky-700 font-semibold text-sm">تعديل</a>
                            <form action="{{ route('admin.homepage-sliders.destroy', $row) }}" method="POST" class="inline" onsubmit="return confirm('حذف هذا السلايدر؟');">
                                @csrf
                                @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new Sortable(container, {
        ...
        onEnd: function () {
            ...
            fetch('{{ route('admin.homepage-sliders.reorder') }}', {
                method: 'POST',
                headers: {
