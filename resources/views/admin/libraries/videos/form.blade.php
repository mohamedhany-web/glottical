    ...
    <form method="POST" id="library-video-form"
        ...
        <article class="rounded-2xl border border-line bg-surface p-5 shadow-soft space-y-4">
            ...
            <div class="rounded-xl border border-dashed border-line bg-canvas-muted/40 p-4 space-y-3">
                ...
                <p class="text-xs text-muted">الرفع يحاول Cloudflare مباشرة، وإن حُجب CORS يكتمل تلقائياً عبر الخادم. قرص: <strong>{{ $uploadDisk ?? 'r2' }}</strong></p>
...
(function () {
    ...
    function putFileViaServer(token, file, onPercent) {
        return new Promise(function (resolve, reject) {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', @json(route('admin.libraries.videos.proxy')), true);
