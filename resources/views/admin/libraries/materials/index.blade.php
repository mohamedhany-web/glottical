...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-muted">
                ملفات المحاضرات وفولدرات معلم×سنة — التخزين على
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-2xl border border-line bg-surface p-4 shadow-soft"><div class="text-xs text-muted">فولدرات</div><div class="mt-1 text-2xl font-semibold">{{ $stats['folders'] ?? 0 }}</div></div>
    </div>
    ...
    <form method="GET" class="grid gap-3 rounded-2xl border border-line bg-surface p-4 shadow-soft md:grid-cols-6">
        ...
        <select name="folder_id" class="{{ $field }}">
            ...
            @foreach(($folders ?? []) as $f)
                <option value="{{ $f->id }}" @selected((string) request('folder_id') === (string) $f->id)>{{ $f->name_ar }}</option>
            @endforeach
        </select>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-canvas-muted text-xs text-muted">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-start">المحاضرة / المجلد</th>
                ...
                <tbody>
                    @forelse($materials as $m)
                        <tr class="border-t border-line">
                            ...
                            <td class="px-4 py-3">
                                @if($m->folder)
                                    <div class="font-medium text-ink">📁 {{ $m->folder->displayName() }}</div>
                                    <div class="text-xs text-muted">{{ $m->folder->instructor?->name }} · {{ $m->folder->academicYear?->name }}</div>
                                @else
